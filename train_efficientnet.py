"""
Ultra-fast training script using pre-extracted EfficientNetB0 features.
Trains a Dense classifier on the extracted features, combines them into 
a single final model, evaluates it, and exports it to ONNX for ensemble inference.
"""
import os
...
print("\n2. Building EfficientNetB0 feature extractor...")
inputs = layers.Input(shape=(224, 224, 3))
# The data generator outputs [0, 1]. EfficientNetB0 natively expects [0, 255] 
# and has a built-in Rescaling layer inside its architecture.
x = inputs * 255.0
base_model = EfficientNetB0(weights='imagenet', include_top=False, input_tensor=x)
base_model.trainable = False

# We want the output of the last convolutional layer (for Grad-CAM if needed)
...
def extract_features(generator, name):
    ...
    for step in range(steps):
        ...
        # Run through EfficientNetB0 to get features
...
# Note: EfficientNetB0 outputs 1280 feature channels from its final conv layer
...
base_final = EfficientNetB0(weights='imagenet', include_top=False, input_tensor=x_final)
